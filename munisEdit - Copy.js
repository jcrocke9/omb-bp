function OrgObjLine(ombpMdOrg, ombpMdObj, ombpMdAmt) {
    ...
    self.ombpMdOrg = ko.observable().extend({ maxLength: 8});
    self.ombpMdObj = ko.observable().extend({ maxLength: 5});
    self.ombpMdAmt = ko.observable();
}
...
//This block successfully pulls data from the service; it does not successfully get the shortTitle() because shortTitle() is a KO 'value' that gets overwritten each time
...
/*    
	...
	$.ajax(
		{
			...
			success: function(data, textStatus) {
					console.log('JoeSuccess', joeCheckUrl, textStatus, data);
				},
	...
	*/
    ...
    self.addOrgobj = function (ombpMdOrg, ombpMdObj, ombpMdAmt) {
        self.orgobj.push(new OrgObjLine(ombpMdOrg, ombpMdObj, ombpMdAmt));
